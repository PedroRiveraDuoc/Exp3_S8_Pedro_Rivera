...
public class UsuarioController {
    ...
    public CollectionModel<EntityModel<Usuario>> getAllUsuarios() {
        ...
        CollectionModel<EntityModel<Usuario>> resource = CollectionModel.of(usuariosResources, linkTo.withRel("usuarios"));
        return resource;
    ...
    */
    ...
    @GetMapping("/{id}")
    public EntityModel<Usuario> getUsuarioById(@PathVariable @Min(1) int id) {
    ...
    */
    ...
    public EntityModel<Usuario> addUsuario(@Validated @RequestBody Usuario usuario) {
        ...
        return EntityModel.of(savedUsuario,
                WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(this.getClass()).getUsuarioById(savedUsuario.getId())).withSelfRel(),
                WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(this.getClass()).getAllUsuarios()).withRel("all-usuarios")
                );
    }
    ...
    */
    ...
    @PutMapping("/{id}")
    public EntityModel<Usuario> updateUsuario(@PathVariable @Min(1) int id, @Valid @RequestBody Usuario usuarioActualizado) {
        Usuario updatedUser = usuarioService.updateUsuario(id, usuarioActualizado);
        return EntityModel.of(updatedUser,
                WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(this.getClass()).getUsuarioById(updatedUser.getId())).withSelfRel(),
                WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(this.getClass()).getAllUsuarios()).withRel("all-usuarios")
                );
    }
